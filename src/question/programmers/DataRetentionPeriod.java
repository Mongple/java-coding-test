...
public class DataRetentionPeriod {
     ...
     * URL : <a href="https://school.programmers.co.kr/learn/courses/30/lessons/150370" />
     * 
     * [문제 설명]
    ...
    public static int[] solution(String today, String[] terms, String[] privacies) {
        ...
        for (int i = 1; i <= privacies.length; i++) {
            String[] privacySplit = privacies[i - 1].split(" ");

            int addMonth = termMap.get(privacySplit[1]);
            int expireDayOfDate = toCustomDay(privacySplit[0])
                    + (addMonth * 28);
            
            if (dayOfDate >= expireDayOfDate) {
                answer.add(i);
            }
        }
