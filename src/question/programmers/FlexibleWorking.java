...
public class FlexibleWorking {
    ...
    /**
     * Programmers. 유연근무제
     * URL : <a href="https://school.programmers.co.kr/learn/courses/30/lessons/388351" />
     *
     * [문제 설명]
    ...
    public static int solution(int[] schedules, int[][] timeLogs, int startDay) {
        ...
        for (int i = 0; i < schedules.length; i++) {
            ...
            successTime = LocalTime.of(schedules[i] / 100, schedules[i] % 100).plusMinutes(10);
            fullSuccess = true;
            ...
            for (int j = 0; j < timeLogs[i].length; j++) {
                ...
                dayOfWeek = DayOfWeek.of(startDay).plus(j);
                
                /*
                 * 월요일이 1일때 다른 방식으로 툐요일 일요일 판단,  토(startday % 7 == 6) 일(startday % 7 == 0)
                 * 월요일이 0일때 다른 방식으로 툐요일 일요일 판단,  토(startday % 7 == 5) 일(startday % 7 == 6)
                 */
                if (!DayOfWeek.SATURDAY.equals(dayOfWeek) && !DayOfWeek.SUNDAY.equals(dayOfWeek)) {
                    diffTargetTime = LocalTime.of(timeLogs[i][j] / 100, timeLogs[i][j] % 100);
                    
                    if (successTime.isBefore(diffTargetTime)) {
                        fullSuccess = false;
                    }
                }
            }
            
            if (fullSuccess) answer++;
        }
